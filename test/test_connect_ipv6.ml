...
module Test_connect_ipv6 (B : Vnetif_backends.Backend) = struct
  ...
  let err_read e =
    let err = Format.asprintf "%a" V.Stackv6.TCPV6.pp_error e in
    failf "Error while reading: %s" err

  let err_write e =
    let err = Format.asprintf "%a" V.Stackv6.TCPV6.pp_write_error e in
    failf "client tried to write, got %s" err

  let accept flow expected =
    let ip, port = V.Stackv6.TCPV6.dst flow in
    Log.debug (fun f -> f "Accepted connection from %s:%d" (Ipaddr.V6.to_string ip) port);
    V.Stackv6.TCPV6.read flow >>= function
    | Error e      -> err_read e
    | Ok `Eof      -> err_read_eof ()
    | Ok (`Data b) ->
  ...
  let test_tcp_connect_two_stacks () =
    ...
    Lwt.pick [
      ...
      (V.create_stack_v6 ~ip:[server_address] backend >>= fun s1 ->
       V.Stackv6.listen_tcpv6 s1 ~port:80 (fun f -> accept f test_string);
       V.Stackv6.listen s1) ;
      ...
      (Lwt_unix.sleep 0.1 >>= fun () ->
       ...
       Lwt.pick [
       V.Stackv6.listen s2;
       (let conn = V.Stackv6.TCPV6.create_connection (V.Stackv6.tcpv6 s2) in
       or_error "connect" conn (server_address, 80) >>= fun flow ->
       Log.debug (fun f -> f "Connected to other end...");

       V.Stackv6.TCPV6.write flow (Cstruct.of_string test_string) >>= function
       | Error `Closed -> err_write_eof ()
       | Error e -> err_write e
       | Ok ()   ->
         Log.debug (fun f -> f "wrote hello world");
         V.Stackv6.TCPV6.close flow >>= fun () ->
